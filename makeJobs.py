...
theConfig = Config("config.json")
theConfig.setupLogger()
...
if len(sys.argv) < 2:
  raise RuntimeError("This script takes a git hash as input.")
gitHash = sys.argv[1]

theResults = Results(theConfig, gitHash)

theCompilers = Compilers(theConfig)
theCompilers.addCompiler("gfortran", "--version")
theCompilers.addCompiler("ifort",    "--version")
theCompilers.addCompiler("nagfor",   "-V")
